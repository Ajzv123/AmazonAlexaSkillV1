...
export class ReadingComponent extends BaseComponent {
  ...
  generateQuestions(analysis: AnalyzedText) {
    ...
    for (const type of entityTypes) {
      const relevantEntities = Array.from(new Set(analysis.entities.filter((entity: AWS.Comprehend.Entity) => entity.Type === type).map((entity: AWS.Comprehend.Entity) => entity.Text)));
